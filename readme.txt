=== Outdoor ===
Contributors: maycorolbuche1
Donate link: https://maycorolbuche.com.br/
Tags: video, image, outdoor
Requires at least: 5.0
Tested up to: 6.6
Stable tag: 1.3.1
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

== Description ==

Este plugin serve para projetar imagens e vídeos na tela de forma dinâmica.

== Screenshots ==

1. Cadastre as imagens e vídeos para serem exibidos no outdoor
2. Projete o outdoor na tela para fazer a divulgação de seus projetos e eventos

== Changelog ==

= 1.3.0 =

Ajuste interno do banco de dados.

= 1.2.0 =

Verificação se o plugin foi instalado corretamente, fornecendo meios para corrigir em caso de instalação problemática.

= 1.1.0 =

Opção para abrir mídia quando clicar sobre o nome ou miniatura.

= 1.0.0 =

Lançamento inicial